...
#	get_node("dialogueNode/Label").id(id)
	...
	get_node("nodes/" + branch + "/dialogue/Label/Edit").set_position(Vector2(10,10))
...
#		var prevnumReplies = node["dialogue"][str(int(currentBranch)-1)]["replies"].size()
#		print(prevnumReplies)

#		for i in prevReplies:
#			var next = node["dialogue"][previousBranch]["replies"][i]["next"]

#			_create_instanced_UI_element(str(i), editorNode, get_node("nodes/" + previousBranch), 300, 100, SCREENSIZE.x/2 -400, SCREENSIZE.y/2 + (i * 125) - offset, 10)
...
#		get_node("nodes/" + branch + "/" + str(i) + "/Label/Edit").set_position(Vector2(10,10))
